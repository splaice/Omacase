...
omacase extras [name]       optional opt-in tweaks: sudo-touchid, clean (never run by install)
...
`herdr` is the agent-aware multiplexer. `omacase install` wires it up in both
directions for every agent CLI it ships:

- **State hooks** — each agent reports `idle`/`working`/`blocked`/`done` back to
  herdr, which drives its agent sidebar and `herdr agent` commands. Managed by
  `herdr integration status|install|uninstall <agent>`.
- **The herdr skill** — generated by `herdr --skill` into the shared
  `~/.agents/skills/herdr/`, then symlinked into each harness that reads a
  `skills/` directory, so agents can drive herdr's panes, tabs, and workspaces.

Both steps are idempotent and re-run on `omacase update`, so a harness installed
later is picked up on the next run. Because herdr's server outlives its client,
